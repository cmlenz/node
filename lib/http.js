...
function connectionListener (connection) {
  ...
  createIncomingMessageStream(connection, function (incoming, should_keep_alive) {
    ...
    //  RFC 2616: "A server MUST NOT send transfer-codings to an HTTP/1.0 client."
    sys.debug(incoming.httpVersion);
    if (incoming.httpVersion == '0.9' || incoming.httpVersion == '1.0') {
